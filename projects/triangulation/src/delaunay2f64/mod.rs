...
/// Represents the area outside of the delaunay2f64.
...
/// will not be included in the delaunay2f64 for robustness.
fn nearly_equals(a: &Point<f64>, b: &Point<f64>) -> bool {
    (a.x - b.x).abs() <= f64::EPSILON * 2.0 && (a.y - b.y).abs() <= f64::EPSILON * 2.0
}
...
/// Result of the Delaunay delaunay2f64.
#[derive(Debug, Clone)]
struct TriangulationState {
    ...
    /// A vector of adjacent halfedge indices that allows traversing the delaunay2f64 graph.
    ///
    ...
    /// A vector of indices that reference points on the convex hull of the delaunay2f64,
    /// counter-clockwise.
    pub hull: Vec<usize>,
}
...
impl Hull {
    fn new(n: usize, center: Point<f64>, i0: usize, i1: usize, i2: usize, points: &[Point<f64>]) -> Self {
    ...
    fn hash_edge(&mut self, p: &Point<f64>, i: usize) {
        let key = self.hash_key(p);
        self.hash[key] = i;
    }

    fn find_visible_edge(&self, p: &Point<f64>, points: &[Point<f64>]) -> (usize, bool) {
...
fn calc_bbox_center(points: &[Point<f64>]) -> Point<f64> {
...
fn find_closest_point(points: &[Point<f64>], p0: &Point<f64>) -> Option<usize> {
    ...
    if min_dist == f64::INFINITY { None } else { Some(k) }
}

fn find_seed_triangle(points: &[Point<f64>]) -> Option<(usize, usize, usize)> {
...
fn handle_collinear_points(points: &[Point<f64>]) -> Triangulation<f64> {
...
/// Returns the delaunay2f64 for the input points.
/// For the degenerated case when all points are collinear, returns an empty delaunay2f64 where all points are in the hull.
pub fn triangulate_2d_f64(points: &[Point<f64>]) -> Triangulation<f64> {
